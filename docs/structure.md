...
- The `/about` landing experience blends Sauti-specific hero copy with the interactive Kenya counties map (`app/javascript/mastodon/features/about/components/kenya_counties_map.jsx`, data in `public/kenya_adm1_full.geojson`). Leaflet assets are vendored locally under `app/javascript/vendor/leaflet/` so the setup works without fetching new npm packages.
